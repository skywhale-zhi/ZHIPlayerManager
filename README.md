...
- 指令9-2： `/vst [name]`
- 功能9-2： 查询这个玩家的所有状态数据，包括上述，仅返回纯文本
-
- 权限10： `zhipm.sort`
- 指令10-1： `/zsort help`
...
- 指令10-4： `/zsort boss [num/all]`
- 功能10-4： 对当前服务器内所有玩家的击杀Boss数进行降序排列， **不填 num 默认只陈列前10名** ，填 num 表示陈列前 num 名，填 all 表示陈列所有排名
- 指令10-4： `/zsort kill [num/all]`
- 功能10-4： 对当前服务器内所有玩家的击杀总生物数进行降序排列， **不填 num 默认只陈列前10名** ，填 num 表示陈列前 num 名，填 all 表示陈列所有排名
- 指令10-4： `/zsort rarenpc [num/all]`
- 功能10-4： 对当前服务器内所有玩家的击杀罕见生物数进行降序排列， **不填 num 默认只陈列前10名** ，填 num 表示陈列前 num 名，填 all 表示陈列所有排名
-
- 权限11： `zhipm.export`
- 指令11： `/zout [name/all]`
...
- 权限13： `zhipm.freeze`
- 指令13-1： `/zfre [name]`
- 功能13-1： 冻结某个玩家，从名称，uuid，和 IP 三个数据进行处理，即使玩家离线也能处理，但在服务器重启后失效
- 指令13-2： `/zunfre [name]`
- 功能13-2： 解冻某个玩家
- 指令13-3： `/zunfre all`
- 功能13-3： 解冻所有被冻结的玩家
